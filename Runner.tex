...
issued by the application~\cite{lwn2015:seccomp}. Several namespace settings also exist
to control access to the network and to other processes outside the sandbox.
...
system for Linux containers. Security of confined processes is controlled via Linux
Seccomp, whereas aggregate resource consumption is managed by the Linux Control Groups
(cgroups) subsystem~\cite{fink2014:docker}. A key feature of Docker absent from most
other container solutions is the use of layered filesystem images: a single operating
system image can be used as a basis for many containers while allowing them to persist
writes in their own private spaces. This design leads to significant storage savings
when multiple containers are created from a same base image~\cite{felter2015:comparison}.
