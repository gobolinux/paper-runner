...
\begin{document}
\title{Dependency Heaven: Conflict Resolution via Containerless Filesystem Virtualization}
...
In short, if a program depends on a given library `/usr/lib/libfoo.so` and it
...
processes under environments with strict security constraints and with
special needs for resource throttling. However, as this paper shown,
they impose significant administration overhead when it comes to using
them to run mundane executables that present versioning conflicts.
...
every program ever released with zero configuration, as long as all related
packages are uncompressed under the versioned directory structure.
